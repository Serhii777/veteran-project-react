...
const PsychologicalHelp = () => {
  ...
  const [psychologicals, setPsychologicals] = useState([]);

  const mounted = useRef(true);

  const getItems = useCallback(() => {
    mounted.current = true;
    if (psychologicals.length && !alert) {
      return;
    }
    ...
    getAllItems().then((items) => {
      ...
      if (mounted.current && items) {
        setPsychologicals(items);
      }
    });

    return () => (mounted.current = false);
  }, [alert, psychologicals]);
  ...
  // console.log("psychologicals:", psychologicals);
    ...
    <Fragment>
      <div className={styles.сontentPage}>
        <div className={styles.сontentPageTitleWrapper}>
          ...
          <h2 className={styles.сontentPageTitle}>Психологічна допомога</h2>
        </div>
        <div className={styles.сontentPageWrapper}>
          ...
          <div className={styles.сontentPageMain}>
            <ul className={styles.сontentPageList}>
              ...
              {psychologicals && psychologicals.length > 0 ? (
                psychologicals.map((item, index) => (
